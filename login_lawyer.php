...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['email']) && isset($_POST['password'])) {
        ...
        $lawyerpassword = $_POST['password']; // Note: Password should be hashed in your database

        // Query your database to find the user based on their email
        $sql = "SELECT lawyerid, email, lawyerpassword FROM lawyeruser WHERE email = '$email'";
        ...
        if ($result) {
            if (mysqli_num_rows($result) == 1) {
                ...
                if (password_verify($lawyerpassword, $row['lawyerpassword'])) {
                    // Password is correct; create a session
                    $_SESSION['user_id'] = $row['lawyerid'];
